...
public class StreamZipFile {
   ...
   */
  private final FSDataInputStream archive;
  ...
  public StreamZipFile(String path, FSDataInputStream in, long fileLength)
      throws IOException {
    this.path = path;
    this.archive = in;
    this.fileLength = fileLength;
    this.encoding = ZipEncodingHelper.UTF8;
  ...
  public InputStream getInputStream(ZipArchiveEntry ze) throws IOException {
    ...
    switch (ze.getMethod()) {
      ...
      default:
        throw new IOException("Found unsupported compression method " + ze.getMethod());
    }
  }
  ...
  protected void finalize() throws Throwable {
    try {
      if (!closed) {
        System.err.println("Cleaning up unclosed ZipFile");
        close();
      }
    } finally {
      super.finalize();
...
//        ZipArchiveOutputStream.ZIP64_EOCD_LOC_SIG);
  ...
  private boolean tryToLocateSignature(long minDistanceFromEnd,
    ...
    boolean found = false;
    long off = fileLength - minDistanceFromEnd;
    final long stopSearching =
        Math.max(0L, fileLength - maxDistanceFromEnd);
    if (off >= 0) {
   ...
   * skipping failed.
   */
  private int skipBytes(final int numBytes) throws IOException {
    long pos = archive.getPos();
    if (pos == fileLength - 1) {
      throw new EOFException();
    }

    long seekPos = pos + numBytes;
    seekPos = Math.min(fileLength - 1, seekPos);
    archive.seek(seekPos);
    return (int)(seekPos - pos);
//    int totalSkipped = 0;
...
//    }
  ...
  private void resolveLocalFileHeaderData(Map<ZipArchiveEntry, NameAndComment>
    ...
    for (Map.Entry<ZipArchiveEntry, OffsetEntry> ent : origMap.entrySet()) {
      ...
      while (lenToSkip > 0) {
        int skipped = skipBytes(lenToSkip);
        if (skipped <= 0) {
